...
frappe.pages['service-scheduling'].on_page_load = function (wrapper) {
	...
	// Build page structure (HTML injected from JS)
	let header_controls = $(`
	...
	let schedule_grid = $(`<div id="schedule-grid"></div>`).appendTo(page.body);
...
function render_schedule_grid(technicians, selected_date, appointments) {
	...
	const headerRow = $(`
		<div class="technician-row header-row" style="display: flex; height: 40px; border-bottom: 1px solid #ddd;">
			<div class="technician-name" style="width: 20%; background: #f0f0f0; text-align: center; line-height: 40px; border-right: 1px solid #ddd;">
	...
	`);
	// Insert bold time labels inside the timeline-cell; loop until m < TOTAL_WORKING_MINUTES (omitting "19:00")
	for (let m = 0; m < TOTAL_WORKING_MINUTES; m += 60) {
		const timeLabel = minutesToTimeString(START_TIME_MINUTES + m);
		const leftPercent = (m / TOTAL_WORKING_MINUTES) * 100;
		const labelDiv = $(`
			<div class="time-label" style="position: absolute; left: ${leftPercent}%; transform: translateX(-50%); font-size: 12px; color: #555; font-weight: bold;">
				${timeLabel}
			</div>
		`);
	...
	technicians.forEach(tech => {
		const techRow = $(`
			<div class="technician-row" data-tech="${tech.name}" style="display: flex; height: 33px; border-bottom: 1px solid #ddd;">
				<div class="technician-name" style="width: 20%; background: #f0f0f0; text-align: center; line-height: 33px; border-right: 1px solid #ddd;">
					${tech.full_name}
				</div>
	...
	// Set top to header row height (40px) + separator (1px) = 41px.
...
function edit_event(eventElement) {
	...
	const d = new frappe.ui.Dialog({
		...
		fields: [
			...
			// Add a Tab Break here called Other Details
...
// Context menu implementation for right-click on events
...
$(document).on('contextmenu', '.schedule-event', function(e) {
	...
	// Only show custom context menu for Dispatched or Scheduled statuses.
	...
	`);
	// Store appointmentId and the event element in the menu data.
...
function render_date_table(selected_date) {
	const date_range = generate_30_day_range(selected_date);
	const month_header = get_month_header(date_range);
	$("#month-row").text(month_header);
...
function generate_30_day_range(selected_date) {
	const dates = [];
...
function get_month_header(dates) {
	const uniqueMonths = [...new Set(dates.map(date => frappe.datetime.str_to_user(date).split(" ")[1]))];
...
function load_schedule(selected_date) {
	...
	frappe.call({
		...
		callback: (r) => {
			if (r.message) {
				const technicians = r.message.technicians;
				const appointments = r.message.appointments;
				render_date_table(selected_date);
				render_schedule_grid(technicians, selected_date, appointments);
			} else if (r.exc) {
