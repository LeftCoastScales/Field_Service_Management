...
class LCSCustomerEquipment(Document):
    ...
    def _validate_model_belongs_to_manufacturer(self):
        ...
        if self.base_scale_model and self.base_manufacturer:
            ...
            if base_model_manufacturer != self.base_manufacturer:
                frappe.throw(
                    frappe._(
                        "Base model {0} belongs to {1}, not {2}."
                    ).format(self.base_scale_model, base_model_manufacturer, self.base_manufacturer)
                )
    ...
    def _validate_base_serial_unique(self):
        ...
        if duplicate:
            frappe.throw(
                frappe._(
                    "Base serial number {0} is already registered for manufacturer {1} "
                    "on record {2}."
    ...
    def _create_paired_base(self):
        ...
        frappe.db.set_value("LCS Customer Equipment", self.name, {
