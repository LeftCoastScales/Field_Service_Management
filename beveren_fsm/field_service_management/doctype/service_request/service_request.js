...
frappe.ui.form.on("Service Request", {
    refresh: function (frm) {
        ...
        // Status Buttons
        if (frm.doc.docstatus === 1 && !["Open", "Converted"].includes(frm.doc.status)) {
        ...
        // Create Button
        if (frm.doc.docstatus === 1 && !["On Hold", "Quotation", "Converted", "Closed"].includes(frm.doc.status)) {
                frm.add_custom_button(__("Service Quotation"), () => {
    ...
    make_service_quotation: frm => {
        frappe.model.open_mapped_doc({
            method: "beveren_fsm.field_service_management.doctype.service_quotation.service_quotation.make_service_quotation",
            frm: frm
        });
    },
    make_order_from_request: frm => {
        frappe.model.open_mapped_doc({
            method: "beveren_fsm.field_service_management.doctype.service_order.service_order.make_order_from_request",
            frm: frm,
            args: {
