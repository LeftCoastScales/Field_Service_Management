...
function lcs_saq_suggest_price(frm, cdt, cdn) {
    ...
    frappe.call({
        method: "beveren_fsm.field_service_management.doctype"
        ...
        callback(r) {
            if (r.message && r.message.price >= 0) {
                ...
                if (suggested === 0) {
                    frappe.show_alert({
                        message: "No default price for this scale type — enter price manually.",
                        indicator: "orange"
                    }, 5);
                }
            }
...
function lcs_saq_recalc_row(frm, cdt, cdn) {
    ...
    frappe.model.set_value(cdt, cdn, "extended_price", Math.round(price * qty * 100) / 100);
...
function lcs_saq_recalc_totals(frm) {
    ...
    const per_service = rows.reduce((sum, r) => sum + (parseFloat(r.extended_price) || 0), 0);
...
function lcs_saq_action_buttons(frm) {
    ...
    if (frm.doc.status === "Accepted" && !frm.doc.linked_service_agreement) {
        frm.add_custom_button("Convert to Service Agreement", () => {
            frappe.confirm(
    ...
    if (frm.doc.linked_service_agreement) {
        frm.add_custom_button("View Service Agreement", () => {
            frappe.set_route("Form", "LCS Service Agreement", frm.doc.linked_service_agreement);
        });
    }
}