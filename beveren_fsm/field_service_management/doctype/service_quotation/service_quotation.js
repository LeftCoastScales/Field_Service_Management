...
frappe.ui.form.on("Service Quotation", {
    ...
    refresh: function (frm) {
        // Status Buttons
        if (frm.doc.docstatus === 1 && !["Ordered", "Converted"].includes(frm.doc.status)) {
        ...
        if (frm.doc.docstatus === 1 &&!["Lost", "Ordered", "Converted"].includes(frm.doc.status)) {
		...
		if (frm.doc.docstatus === 1 && !["Lost", "Closed", "On Hold", "Ordered", "Converted"].includes(frm.doc.status)) {
    ...
    disable_creating_order: frm => {
		if(!["Lost", "On Hold", "Closed", "Ordered", "Converted"].includes(frm.doc.status)){
			return
		}
		else{
	...
	make_order_from_quote: frm => {
		frappe.model.open_mapped_doc({
			method: "beveren_fsm.field_service_management.doctype.service_order.service_order.make_order_from_quote",
			frm:frm
		});
    },
...
beveren_fsm.field_service_management.ServiceQuotationController = class ServiceQuotationController extends erpnext.selling.SellingController {
    onload(doc, dt, dn) {
...
cur_frm.script_manager.make(beveren_fsm.field_service_management.ServiceQuotationController);
...
frappe.ui.form.on("Service Quotation Item", "stock_balance", function (frm, cdt, cdn) {
