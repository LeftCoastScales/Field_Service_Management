...
**Why Opus:** DCC is an external, PTB-maintained international standard for machine-readable calibration certificates (measurement results, uncertainty, traceability chain, digital signature). Getting the mapping and validation approach wrong means rework once customers start requesting DCC-format certs alongside the PDF, and this phase likely shares data with Phase 7B (ISO 17025 QMS).

**Key questions:**
- Which doctype(s) hold the calibration/measurement results that map to the DCC schema's required elements?
- Is DCC XML generated alongside the existing PDF certificate, or only on customer request?
- What satisfies the schema's digital signature / authenticity requirement?
- How does this phase sequence relative to Phase 7B, given the shared calibration-record data model?
